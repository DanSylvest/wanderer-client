<template>
    <context-menu :c-activated.sync="localShow" :c-offset-x="offset.x" :c-offset-y="offset.y"
        ...
        <context-menu-item v-if="userOnlineCharacters.length > 0" c-title="Waypoints" c-icon="call_split"
                           :c-is-submenu="true" v-show="isSystemInKSpace">
            <context-menu-item v-for="{name, charId} in userOnlineCharacters" :c-is-submenu="true" :c-title="name"
                               :key="charId">
                <context-menu-item c-title="Set Destination" c-icon="near_me" @click="onSetDestination(charId)"/>
                <context-menu-item c-title="Add Waypoint Front" c-icon="call_missed"
                                   @click="onAddWaypointFront(charId)"/>
                <context-menu-item c-title="Add Waypoint Back" c-icon="call_missed_outgoing"
        ...
        <context-menu-item c-title="Unmark as hub" c-icon="near_me_disabled" v-show="!markAsHub"
                           @click="onMarkAsHub(false)"/>

        <context-menu-item c-title="Unlock system" c-icon="lock_open" v-show="isLocked"
        ...
        <context-menu-item c-title="Remove system" c-icon="delete" c-icon-class="fg-negative" v-show="!isLocked"
